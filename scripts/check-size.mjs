...
const routeNames = ["explore", "review"];
...
export function measureAssetSizes(root = resolve(process.cwd(), "dist")) {
    ...
    const routes = Object.fromEntries(routeNames.map((route) => {
    ...
    return { routes, sharedReactClientJs: Math.max(...routeNames.map((route) => routes[route].clientJs)) };
}

export function assertSizeBudgets(actual, budgets) {
    const checks = routeNames.flatMap((route) => [
