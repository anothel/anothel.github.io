export const topicDefinitions = [
    ...
    export const trackedTopicLabels = topicDefinitions.map((topic) => topic.label);
    export const topicPageLabels = ["AI agents", "MCP", "Agent skills", "AI evals", "AI engineering", "Workflow automation", "Security"];

    export function topicBySlug(slug) {
        const topic = topicByLabel(slug);
        return topic && topicPageLabels.includes(topic.label) ? topic : null;
    }

    export function topicNotes(prefix = "") {
        return topicPageLabels.map((label) => {
            const topic = topicByLabel(label);
            return {
                topic: label,
                description: topic?.description || "",
                route: routeForTopic(label, prefix),
                exploreRoute: exploreRouteForTopic(label, prefix),
                note: topic?.note
            };
