...
test("current Home, Notes, topic, Explore, and Review assets stay within reviewed raw-byte budgets", () => {
    ...
    assert.equal(result.actual.sharedCss.asset, "css/site.css");
    assert.ok(result.actual.sharedCss.size <= 60000);
...
test("asset size gate enforces the shared CSS ceiling", () => {
    const actual = checkSize().actual;
    const budgets = JSON.parse(readFileSync("asset-size-budgets.json", "utf8"));
    budgets.sharedCss = actual.sharedCss.size - 1;
    assert.throws(
        () => assertSizeBudgets(actual, budgets),
        new RegExp(`shared CSS \\(css/site\\.css\\): ${actual.sharedCss.size} bytes exceeds ${budgets.sharedCss} bytes`)
    );
});
