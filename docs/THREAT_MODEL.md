...
- Generated data, manifest, canonical site routes, sitemap output, and Astro content output drift apart.
...
- Published compatibility-only browser renderers use `js/safe-dom.js` helpers to escape HTML and restrict hrefs; native routes do not consume that renderer set.
...
- `scripts/data-contract.mjs` validates checked-in JSON; `scripts/check-dist.mjs` validates built routes/assets/data/internal links against the canonical site route model.
- Node renderer/security tests and Playwright accessibility/mobile/smoke tests run through `npm run check`.
- Scoped architecture tests block dynamic script/global bridges, checked-in route artifacts, React hydration on static content routes, and retired pass-through assets. Raw-byte budgets cap generated Home/Notes/topic/Explore/Review growth.
- Source metadata exposes `ok`, `partial`, `fallback`, `error`, rate limits, and stale-but-safe reuse.
- Data refresh validates before committing generated JSON only; workflow concurrency prevents overlapping refresh commits.
...
| 404, robots, sitemap | `src/pages/404.astro`, `src/pages/robots.txt.ts`, and `src/pages/sitemap.xml.ts` | Astro escaping for 404 text, fixed robots policy, canonical route/origin model, checked-in manifest date, and generated-artifact validation. |
| Data/JS/CSS endpoints | Astro build-time endpoint routes | Fixed allowlists in route source; no request-time filesystem access after static build. Published compatibility-only JS renderers have no native route consumers. |

`js/data-health.js` and `js/signal-schema.js` remain active internal modules. The other renderer modules stay published only to preserve existing `/js` URLs; remove them only after a production reference/usage audit and explicit compatibility-breaking approval.
